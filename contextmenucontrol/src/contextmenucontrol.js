...
ContextMenuControl.prototype.searchDirs_ = function(letter) {
  ...
  if (me.actual) {
   ...
   var icon = new GIcon();
   icon.image = "http://maps.google.com/mapfiles/marker" + letter + ".png";
   me.addIconAttr_(icon);
...
ContextMenuControl.prototype.removeDest_ = function() {
  ...
  if (d.length == 0) {
   ...
   me.map_.closeInfoWindow();
   me.map_.removeOverlay(this.poly_);
  }
  else if (d.length > 1) {
...
ContextMenuControl.prototype.addDest_ = function() {
  ...
  if (me.dirmarks_.length == 1) {
   if (me.dirmarks_[0].letter == "A") {
    me.setChosen_("B");
   } else {
    me.setChosen_("A");
   }
   waypoints[0] = me.clickedPoint_;
   waypoints[1] = me.dirmarks_[0].getLatLng();
  }
  // Adds further destinations
  else if (me.dirmarks_.length > 1) {
   me.setChosen_();
   for (var i = 0; i < me.dirmarks_.length; i++) {
    waypoints[i] = me.dirmarks_[i].getLatLng();
   }
...
ContextMenuControl.prototype.setChosen_ = function(letter) {
  if (letter == "A") {
   this.chosen = { start:true, end:false };
  }
  else if (letter == "B") {
   this.chosen = { start:false, end:true };
  }
  else {
   this.chosen = { start:false, end:false };
  }
};
...
ContextMenuControl.prototype.makeDirMarker_ = function(point, letter) {
  ...
  var waypoints = [];
  var icon = new GIcon();
  icon.image = "http://maps.google.com/mapfiles/marker_green" + letter + ".png";
  me.addIconAttr_(icon);
  ...
  GEvent.addListener(marker, "dragend", function() {
   if (d.length == 2) {
    var sticky = (me.chosen.start) ? d[d.length-1].getLatLng(): d[0].getLatLng();
...
ContextMenuControl.prototype.createRouteInfo = function(info, nr, i) {
  ...
  if (nr == 0) {
   blink.style.color = "#a5a5a5";
   blink.style.textDecoration = "none";
   blink.removeAttribute("href");
   blink.style.cursor = "default";
  }
